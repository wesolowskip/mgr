...
INTERVAL=6
export CUFILE_ENV_PATH_JSON=/home2/faculty/pwesolowski/mgr/benchmark-hdd/cufile.json # export is crucial!
# export CUFILE_LOGGING_LEVEL=TRACE
# export CUFILE_LOGFILE_PATH=/home2/faculty/pwesolowski/mgr/gdsio-test.log

RESULTS_DIR="benchmark-$X-$I-$W-$SIZE"
mkdir -p "${RESULTS_DIR}"

/usr/local/cuda/gds/tools/gdsio -D /home2/faculty/pwesolowski/mgr/hdd_test/data -d 0 -w "$W" -s "$SIZE" -i 1M -x "$X" -I "$I" -T 60 &
PID=$!
sleep 1
